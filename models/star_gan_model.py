...
class Resnet_Block(Model):
    ...
    def __init__(self, dim):
        super().__init__()
        self.dim = dim
        self.conv1 = Conv2D(self.dim, 3, 1, 'same')
        # self.bn1 = BatchNormalization()
        self.bn1 = InstanceNormalization()
        self.relu1 = ReLU()
        self.conv2 = Conv2D(self.dim, 3, 1, 'same')
...
class Star_Generator(Model):
    def __init__(self):
        super().__init__()
        
        self.c_dim = 4  # Domain vector(3) + opened/closed(1)
        
        self.down_conv = Sequential([
            Conv2D(64, 3, 1, 'same'),
        ...
        self.resblock = Resnet_Block(256)
        self.up_conv = Sequential([
            Conv2DTranspose(128, 4, 2, 'same'),
            InstanceNormalization(),
            ReLU(),
            Conv2DTranspose(64, 4, 2, 'same'),
            InstanceNormalization(),
            ReLU(),
            Conv2D(3, 1, 1, 'same', activation='sigmoid')
        ])
        
    def call(self, x, c):
        # 36x60x3 + 36x60xc_dim -> 36x60x(3+c_dim)
        # c_block = c
        # c_block = tf.reshape(c_block, (c_block.shape[0], 1, 1, -1))
        # c_block = tf.repeat(input = c_block, repeats=[x.shape[1]], axis=1)
        # c_block = tf.repeat(input = c_block, repeats=[x.shape[2]], axis=2)
        
        x = tf.concat([x, c], axis = 3)

        # Down sampling # 36x60x(3+c_dim) -> 9x15x256
        x = self.down_conv(x)
        
        # BottleNeck    # 9x15x256 -> 9x15x256
        for i in range(4):
            x = self.resblock(x)

        # Up sampling   # 9x15x256 -> 36x60x3
        x = self.up_conv(x)

        return x

class Star_Discriminator(Model):
    def __init__(self):
        super().__init__()
        ...
        self.domain_layer = Sequential([
            BatchNormalization(),
            LeakyReLU(0.2),
            Dropout(0.2),
            Dense(self.c_dim-1, 'softmax')
        ])
        self.state_layer = Sequential([
            BatchNormalization(),
            LeakyReLU(0.2),
            Dropout(0.2),
            Dense(1, 'sigmoid')
    ...
    def call(self, x):
        ...
        output = self.dis_layer(main)
        domain = self.domain_layer(main)
        state = self.state_layer(main)       
        
        return output, domain, state
